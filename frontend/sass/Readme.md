...
folder are mapped to the application's root namespace, 'TrxResearch'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "TrxResearch"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'TrxResearch' in the same way
as `"TrxResearch/sass/src"`.
